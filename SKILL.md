...
Exit codes: `0` success (including empty selection); `1` usage error, coverage-command failure, unreadable source, or parse error; `2` Quality gate.
...
2. Finds `.ts` `.tsx` `.mts` `.cts` files, skipping `node_modules`, `dist`, `build`, `coverage`, `.git`, `target`, and tests
